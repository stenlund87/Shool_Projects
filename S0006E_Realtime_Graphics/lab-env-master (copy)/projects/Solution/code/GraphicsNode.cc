...
{
	//meshRes = std::make_shared<MeshResource>(*MeshResource::Load("D:\\cube.obj"));
	meshRes = std::make_shared<MeshResource>(*MeshResource::Load("D:\\sword.obj"));
...
void GraphicsNode::InitRasterizer()
{
	meshRes = std::make_shared<MeshResource>(*MeshResource::SetupQuad(1.0f));
	texture = std::make_shared<TextureResource>();
	shaderObj = std::make_shared<ShaderObject>();
	shaderObj->LoadShader("D:\\verticeShader.txt", "D:\\ps.txt");
}
