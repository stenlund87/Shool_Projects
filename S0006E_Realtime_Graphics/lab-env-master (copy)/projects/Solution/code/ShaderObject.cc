...
ShaderObject::ShaderObject(LightNode lighty)
{
	light = lighty;
}
...
void ShaderObject::LoadShader(const char *vertex_path, const char *fragment_path)
{
	...
	{
		GLchar* buf = new GLchar[logLength];
		glGetShaderInfoLog(fShader, logLength, NULL, buf);
		printf("[SHADER COMPILE ERROR]: %s", buf);
		delete[] buf;
	}
...
void ShaderObject::ModifyVector(char* vecName, float* pos)
{
	...
	glUniform3fv(location.find(vecName)->second, 1, pos);
}

void ShaderObject::ModifyFloat(char* vecName, GLfloat pos)
{
	if (location.find(vecName) == location.end())
	{
		location[vecName] = glGetUniformLocation(this->program, vecName);
	}
	glUniform1f(location.find(vecName)->second, pos);
}