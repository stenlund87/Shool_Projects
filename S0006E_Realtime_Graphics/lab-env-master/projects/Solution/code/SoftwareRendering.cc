...
SoftwareRendering::SoftwareRendering(Matrix4D viewProj, TextureResource* t)
{
	...
	unsigned char* temp = stbi_load("Zelda3.jpg", &textureWidth, &textureHeight, &bitsPerPixel, 3); // 3 means force rgb, ignore alpha
	...
	zBuffer = std::vector<float>(height*width, 1.0f);

	PixelColor = [this](Vector4D pos, Vector4D uv, Vector4D norm, Matrix4D rotation)->Color
	{
        float x = uv[0], y = uv[1];
		uv.Get(0) >= 1 ? uv.SetX(0.999f) : uv.SetX(uv[0]);
		uv.Get(1) >= 1 ? uv.SetY(0.9999f) : uv.SetY(uv[1]);
		uv.Get(0) <= 0 ? uv.SetX(0.0f) : uv.SetX(uv[0]);
		uv.Get(1) <= 0 ? uv.SetY(0.0f) : uv.SetY(uv[1]);
		norm = (rotation * norm);
		norm.Normalize();
...
void SoftwareRendering::Interpolate(Vertex A, Vertex B, Vertex C, int x, int y)
{
	if (x > 0 && y > 0 && x < width && y < height)
	{
		...
		Vertex vertex(x,y,(A.pos[2] * BCP) + (B.pos[2] * ACP) + (C.pos[2] * ABP));
		//if (vertex.pos[2] > zBuffer[y * width + x])
		//{
		//	return;
		//}
		//zBuffer[y * width + x] = vertex.pos[2];
