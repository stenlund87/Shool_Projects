...
bool Animation::SetupNax3(const char *filename)
{
    ...
    if(naxHead->numClips > 0)
    {
        ...
        for(int clipIndex = 0; clipIndex < numClips; clipIndex++)
        {
            ...
            ptr += sizeof(Nax3Clip);
            //std::cout << naxClip->numCurves << std::endl;
//            std::cout << naxClip->keyDuration << std::endl;
            //this->clip[clipIndex] = *naxClip;


            Nax3Group Clip;
            Clip.clip = *naxClip;
            Clip.curvelist = new Nax3Curve[naxClip->numCurves];
            Clip.clip.keyDuration = Clip.clip.keyDuration ;
...
void Animation::SetupAnimation(int index, Skeleton* skeleton, float dt)
{
   ...
   Key nextRotationKey;
    ...
    for(int i = 0; i < skeleton->joints.size(); i++)
    {
        ...
        rotationMatrix.Set_Position(translation * 0.2f);
