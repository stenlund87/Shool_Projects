...
Skeleton::Skeleton()
{
    ...
    shader->LoadShader("verticeShader.txt", "ps.txt");

    LightNode temp;
//    temp.SetPos(0.0f, 1.0f, -1.0f);

//    temp.SetAmb(0.4f, 0.4f, 0.4f);
//    temp.SetDiff(0.4f, 0.4f, 0.4f);
//    temp.SetSpec(1.0f, 1.0f, 1.0f);
//    temp.SetIntensity(10.0f);
    shader->light = temp;
}
...
void Skeleton::ReadFromFile(char *filename)
{
        ...
        child != 0; child = child->NextSiblingElement())
    {
        ...
        rotationMatrix.Set_Position(positionVector);
...
//Calculate the bind pose position relative to its parent
void Skeleton::CalculateOriginal()
{
    bindPose = joints;
    for(int i = 1; i<bindPose.size(); i++)
    {
        if(bindPose[i].parentIndex == -1)
        {
            continue;
        }
        else
        {
            bindPose[i].transformMatrix = bindPose[bindPose[i].parentIndex].transformMatrix * bindPose[i].transformMatrix;

        }
    }
    for(int j = 0; j < bindPose.size(); j++)
    {
        bindPose[j].transformMatrix = bindPose[j].transformMatrix.Transpose();
    }
}

//Calculate the rotated matrix position relative to its parent
...
//Insert a 4x4 matrix to a linear array
...
//Set the rotation for teh skeleton
...
//Draw the skeleton with animation
void Skeleton::DrawAnimations()
{
    shader->Apply();

//    shader->ModifyVector("lightPos", shader->light.GetPos().GetVector());
//    shader->ModifyVector("ambientColor", shader->light.GetAmb().GetVector());
//    shader->ModifyVector("diffuseColor", shader->light.GetDiff().GetVector());
//
//    shader->ModifyVector("specColor", shader->light.GetSpec().GetVector());
//    shader->ModifyFloat("shininess", shader->light.GetIntensity());

    Calculate();
    //ConvertJointsToArray();
    for(int p = 0; p < tempJoints.size(); p++)
    {
        float arr[16];
        Convert(arr, tempJoints[p].transformMatrix);
...
//Calculates bind pose matrix * animated joint matrix and insert it to a float array[21][16]
void Skeleton::ConvertJointsToArray()
{
    ...
    {
        float arr[16];
        Matrix4D temp =  this->bindPose[i].transformMatrix.Inverse() * this->tempJoints[i].transformMatrix;
        Convert(arr, temp.Transpose());
        for(int j = 0; j < 16; j++)
        {
            mat[i][j] = arr[j];
        }
