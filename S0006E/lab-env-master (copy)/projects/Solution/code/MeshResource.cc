...
MeshResource* MeshResource::SetupQuad(float width)
{
	MeshResource* temp = new MeshResource();
	temp->verticeBuffer = new Vertex[4];
	//Vertex vul(-0.5f, +0.5f, +0.0f);
	//Vertex vur(+0.5f, +0.5f, +0.0f);
	//Vertex vll(-0.5f, -0.5f, +0.0f);
	//Vertex vlr(+0.5f, -0.5f, +0.0f);
	float w = width * 0.5f;
	...
	vul.uv = Vector4D(0, 0, 0);
	vur.uv = Vector4D(1.0f, 0 ,0);
	vll.uv = Vector4D(1.0f, 1.0f,0);
	vlr.uv = Vector4D(0,1.0f,0);

	vul.normal = Vector4D(0, 0, 1);
	vur.normal = Vector4D(0, 0, 1);
	vll.normal = Vector4D(0, 0, 1);
	vlr.normal = Vector4D(0, 0, 1);

	temp->verticeBuffer[0] = vul;
	temp->verticeBuffer[1] = vur;
	temp->verticeBuffer[2] = vll;
	temp->verticeBuffer[3] = vlr;
	temp->numVertices = 4;
	temp->indiceBuffer = new int[6];
	temp->indiceBuffer[0] = 1;
	temp->indiceBuffer[1] = 0;
	temp->indiceBuffer[2] = 3;
	temp->indiceBuffer[3] = 3;
	temp->indiceBuffer[4] = 2;
	temp->indiceBuffer[5] = 1;
	temp->sizeOfBuffer = 6;
	temp->SetBO(temp->verticeBuffer, 6);
	return temp;
}
...
MeshResource* MeshResource::Load(const char* filename)
{
	...
	while (true)
	{
		...
		else if (strcmp(lineHeader, "f") == 0)
		{
			...
			if (matches == 9)
			{
				test[vertexIndex[0] - 1].normal = temp_normals[normalIndex[0] - 1];
				test[vertexIndex[1] - 1].normal = temp_normals[normalIndex[1] - 1];
				test[vertexIndex[2] - 1].normal = temp_normals[normalIndex[2] - 1];

				test[vertexIndex[0] - 1].uv = temp_uvs[uvIndex[0] - 1];
				test[vertexIndex[1] - 1].uv = temp_uvs[uvIndex[1] - 1];
				test[vertexIndex[2] - 1].uv = temp_uvs[uvIndex[2] - 1];
	...
	mesh->verticeBuffer = new Vertex[test.size()];
	mesh->indiceBuffer = new int[vertexIndices.size()];

	for (int i = 0; i < vertexIndices.size(); i++)
	{
		mesh->indiceBuffer[i] = vertexIndices[i];
	}


	for (int k = 0; k < test.size(); k++)
	{
		mesh->verticeBuffer[k] = test[k];
		
	}
	mesh->sizeOfBuffer = vertexIndices.size();
	mesh->numVertices = test.size();
	mesh->SetBO(mesh->verticeBuffer, vertexIndices.size());
	return mesh;
}