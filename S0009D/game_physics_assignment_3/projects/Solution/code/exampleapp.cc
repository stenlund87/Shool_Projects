				...
				if(intersect[j+1])
				{
                    for(int k = 0; k < 6; k++)
                    {
                        if(quads[j].plane[k].center[2] > quads[j+1].plane[k].center[2])
                        {
                            ...
                            quads[j+1].physics.force = ray_wor*10;
...
//					quads[j].intersectPoint = point;
					quads[j].physics.force = ray_wor*10;
//                    quads[j].intersectPoint = globalKingMasterEliteIntersect1337;
...
ExampleApp::Run()
{
    ...
    quads[0].SetPosition(Vector4D(0,0,-5.0f));
	quads[1].SetPosition(Vector4D(2.0f,0,-5.0f));
	...
	while (this->window->IsOpen())
	{
		...
		quads[1].translation.Set_Position(Vector4D(2.0f + xMove,-zMove,-5.1f ));
