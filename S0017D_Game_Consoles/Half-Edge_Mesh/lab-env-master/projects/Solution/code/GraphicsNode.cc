...
void GraphicsNode::InitNode()
{
	...
	meshRes = std::make_shared<MeshResource>(*MeshResource::Load("cube.obj"));
...
void GraphicsNode::InitHEM()
{
	hem = std::make_shared<HE_Mesh>(*HE_Mesh::LoadFromFile("cube.obj"));
	texture = std::make_shared<TextureResource>();
	shaderObj = std::make_shared<ShaderObject>();
