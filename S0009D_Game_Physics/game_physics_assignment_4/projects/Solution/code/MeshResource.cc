...
{
    this->physics.inverseInertiaTensor = this->rotation * this->physics.shapeMatrix * this->rotation.Transpose();
}


void MeshResource::InverseInertiaTensor()
{
    this->physics.inverseInertiaTensor = (this->rotation * this->physics.shapeMatrix.Inverse()) * this->rotation.Transpose();
}
...
void MeshResource::Update(Vector4D collision, float dt)
{
    ...
    this->physics.velocity = this->physics.momentum*(1.0f/this->physics.mass);
    Vector4D newPos;
    newPos = currentPos + this->physics.velocity*dt;
	SetPosition(newPos);
    ...
    Vector4D q = u*(h*u);
    this->physics.torque = ((q - this->physics.centerOfMass).CrossProduct(this->physics.force)*dt);
    this->physics.angularMomentum = (this->physics.angularMomentum + this->physics.torque);
    this->physics.spinVector = (this->physics.inverseInertiaTensor * this->physics.angularMomentum);
    this->physics.skewMatrix = this->physics.skewMatrix.Skew(this->physics.spinVector);

    this->rotation  = ((this->physics.skewMatrix * this->rotation))*dt*0.5f + this->rotation*Matrix4D();
    this->rotation.GramSchmitOrto();
...
{
	selected = false;
    this->physics.momentum = Vector4D();

    this->physics.velocity = Vector4D();
    this->physics.torque = Vector4D();
    this->physics.angularMomentum = Vector4D();
    this->physics.spinVector = Vector4D();
    this->physics.skewMatrix = Matrix4D();
