...
public:
	...
	*/
    ...
    {
        return ((this->coordinates[0] < right.coordinates[0]) && (this->coordinates[1] < right.coordinates[1]) && (this->coordinates[2] < right.coordinates[2]));
...
inline Vector4D Vector4D::Normalize()
{
	...
	{
		length = 1.0f / Length();
		norm.coordinates[0] = coordinates[0] * length;
		norm.coordinates[1] = coordinates[1] * length;
		norm.coordinates[2] = coordinates[2] * length;
        norm.coordinates[3] = 1.0f;
	}
	return norm;
}
...
inline
void Matrix4D::GramSchmitOrto()
{
    Vector4D x1axis=Vector4D(this->matrix[0][0],this->matrix[0][1],this->matrix[0][2],0);
    Vector4D y1axis=Vector4D(this->matrix[1][0],this->matrix[1][1],this->matrix[1][2],0);
    Vector4D z1axis=Vector4D(this->matrix[2][0],this->matrix[2][1],this->matrix[2][2],0);

    Vector4D u1 = Vector4D(0,0,0,0),u2,u3;

    u1 = x1axis.Normalize();
    u2 = (y1axis-(u1*(y1axis*u1))).Normalize();
    u3 = (z1axis-(u1*(z1axis*u1))).Normalize();
    u3  = u3-(u2*(u3*u2));
    u3 = u3.Normalize();
    u1.coordinates[3] = 0;
    u2.coordinates[3] = 0;
    ...
    *this = Matrix4D(u1,u2,u3,Vector4D(0,0,0,1));
